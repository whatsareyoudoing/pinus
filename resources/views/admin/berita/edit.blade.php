...
<form action="{{ asset('admin/berita/edit_proses') }}" method="post" enctype="multipart/form-data" accept-charset="utf-8">
    ...
    <div class="row form-group">
        ...
        <div class="col-md-2">
            <input type="text" name="tanggal_post" class="form-control tanggal" placeholder="Tanggal Post"
                value="<?php if (isset($_POST['tanggal_post'])) {
                    echo old('tanggal_post');
                } else {
                    echo date('Y-m-d', strtotime($berita->tanggal_post));
                } ?>" data-date-format="dd-mm-yyyy">
        </div>
        <div class="col-md-2">
            <input type="text" name="jam_post" class="form-control time-picker" placeholder="Jam post"
                value="<?php if (isset($_POST['jam_post'])) {
                    echo old('jam_post');
                } else {
                    echo date('H:i:s', strtotime($berita->tanggal_post));
                } ?>">
        </div>
